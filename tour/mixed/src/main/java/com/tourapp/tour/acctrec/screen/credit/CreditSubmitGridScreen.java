...
public class CreditSubmitGridScreen extends CreditBaseGridScreen
{
    ...
    public void addListeners()
    {
        ...
        int iTrxStatusID = recTrxStatus.getTrxStatusID(TransactionType.ACCTREC, CreditCard.kCreditCardFile, CreditCard.SUBMITTED);
        String strTrxClass = Integer.toString(iTrxStatusID);
        
        this.getMainRecord().getField(CreditCard.kPaid).addListener(new MoveOnChangeHandler(this.getMainRecord().getField(CreditCard.kDateSubmitted), this.getScreenRecord().getField(McoScreenRecord.kToday)));
        ...
        recTrxStatus.getTrxStatusID(TransactionType.ACCTREC, CreditCard.kCreditCardFile, CreditCard.ENTERED);
